...
const totalDebt = computed(() => {
  return paidDebts.value.reduce(
    (total, debt) => total + debt.remaining_balance,
    0
  );
});
  ...
  if (confirm("Are you sure you want to delete this debt?")) {
    Inertia.delete(route("debts.destroy", debtId), {
      onSuccess: () => {
        ...
        paginatedDebts.value = paginatedDebts.value.filter(
          (debt) => debt.id !== debtId
        );
      },
      onError: () => {
...
<template>
  ...
  <AuthenticatedLayout>
    ...
    <div
      v-if="paidDebts.length > 0"
      class="mt-12 max-w-6xl mx-auto hidden sm:block"
    >
      ...
      >
        ...
        >
          ...
          <tbody>
            ...
            >
              <th scope="row">
                <UnderlineLink :href="route('debts.show', debt.id)">{{
                  debt.debt_name
                }}</UnderlineLink>
              </th>
              <td class="px-6 py-2">
                <span
                  v-if="Number(debt.remaining_balance) === 0"
                ...
                <div v-else>
                  ...
                  <p>
                    ${{ debt.remaining_balance }} remaining out of ${{
                      debt.debt_amount
                    }}
                  </p>
                </div>
              ...
              <td class="px-6 py-4">
                <DateDisplay :date="debt.debt_start_date" />
              </td>
              <td class="m-2">
    ...
    <div class="p-2 block sm:hidden">
      ...
      >
        <div
          class="w-full max-w-6xl p-3 bg-white border border-gray-200 rounded-lg shadow dark:hover:bg-gray-700 hover:bg-gray-300 dark:bg-gray-800 dark:border-gray-700 flex justify-between items-center"
        >
          <div class="min-w-0">
            <h5
            ...
            <span
              v-if="Number(debt.remaining_balance) === 0"
              class="text-white p-2 dark:text-green-400 bg-green-700 rounded-lg px-2 py-1"
              >Paid</span
            >
            <div v-else>
              <ProgressBar :percentage="percentageComplete(debt)" />
