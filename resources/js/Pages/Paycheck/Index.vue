...
const filteredPaychecks = computed(() => {
  return props.paychecks.filter(paycheck => {
    return paycheck.paycheck_date.toLowerCase().includes(searchQuery.value.toLowerCase());
  });
});
  ...
  if (confirm('Are you sure you want to delete this paycheck?')) {
    Inertia.delete(route('paychecks.destroy', paycheckId), {
      onSuccess: () => {
        ...
        props.paychecks = props.paychecks.filter(paycheck => paycheck.id !== paycheckId);
      },
      onError: () => {
        // Handle error
  ...
  <AuthenticatedLayout>
    <template #header>
      <div class="flex justify-between items-center">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Paychecks</h2>
    ...
    <div class="p-2">
      ...
      <div class="mt-2 relative overflow-x-auto mx-auto max-w-6xl rounded-lg hidden sm:block">
        <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
            ...
            <tbody>
              <tr v-for="paycheck in paginatedPaychecks" :key="paycheck.id" 
